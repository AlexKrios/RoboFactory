namespace RoboFactory
{
    public static class Constants
    {
        public const string MusicSourceKey = "musicSource";
        public const string MainCameraKey = "mainCamera";
        public const string UiCameraKey = "uiCamera";
        public const string HudParentKey = "hudParent";
        public const string ScreensParentKey = "screensParent";
        public const string PopupsParentKey = "popupsParent";

        public const string SignInKey = "SignIn";
        public const string SignUpKey = "SignUp";
        public const string VerificationKey = "Verification";

        public const string TestEmail = "test@gmail.com";
        public const string TestPassword = "123456";

        public const int MinStar = 1;
        public const int MaxStar = 2;

        public const int MaxPart = 4;
    
        public const int ProductTypeCount = 4;
    }
}